...
const postBinannce = async (
  ...
  //   }
  // }
  const decimical = await getPrice(symbol)
  const numberDecimical = parseInt(
    decimical.toString().match(/\.(\d*)/)[1].length
  )
  ...
  if (type === 'LIMIT') {
    params = {
      ...
      stopprice: parseFloat(takeprofit).toFixed(numberDecimical),
      timeInForce: 'GTE_GTC',
      timestamp: timestamp,
      closePosition: true,
    ...
    params = {
      ...
      // positionSide: positionSide,
      type: 'STOP_MARKET',
      stopPrice: parseFloat(stopPriceCal).toFixed(numberDecimical),
      timeInForce: 'GTC',
      timestamp: timestamp,
      closePosition: true,
      workingType: 'MARK_PRICE'
...
const getLeverageInitial = async (symbol) => {
  const timestamp = Date.now()
  const params = {
    symbol: symbol,
    timestamp: timestamp
...
const getNotionalLv = async (symbol) => {
  const timestamp = Date.now()
